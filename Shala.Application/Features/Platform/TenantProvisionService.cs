...
public class TenantProvisionService : ITenantProvisionService
{
    ...
    public async Task<(bool Success, object Data)> GetTenantUsersAsync(int tenantId)
    {
        if (tenantId <= 0)
            return (false, new { message = "Invalid tenant." });
        ...
        var users = await _repository.GetTenantUsersAsync(tenantId);

        var result = new List<TenantUserResponse>();
        ...
        result = result
            .OrderByDescending(x => x.Role == "SchoolAdmin")
            .ThenBy(x => x.FullName)
            .ToList();
